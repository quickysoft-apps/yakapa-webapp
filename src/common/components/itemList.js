...
class ItemList extends React.Component {
  ...
  _buildListItems(items) {    
    const listItems = []
    items.map((item, index) => listItems.push(<List.Item key={index}>{item}</List.Item>))    
    return (
  ...
  _buildSkeleton(count) {
    ...
    for (var index = 0; index < count; index++) {
      listItems.push(<Image src={this.props.skeleton} size='medium' centered spaced />)
    }    
    return listItems
  }
  ...
  _buildItems(items) {
    const listItems = []
    items.map((item, index) => listItems.push(this.props.itemTemplate(item)))    
    return listItems
  }
